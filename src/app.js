      ...
      '<div><span class="label">How much in each bag or tub?</span>' +
        ...
        '<div class="chips" style="margin-top:10px">' + weightChips +
          (d.weightG > 0 && d.count > 0
            ? '<button class="chip" data-act="set-weight" data-g="0">Don&rsquo;t weigh it</button>'
            : '') +
        '</div>' +
      '</div>' +
      ...
      '<div>' + (d.showCount
        ? '<span class="label">How many pieces in each bag?</span>' +
          ...
          '<div class="chips" style="margin-top:10px">' +
            ...
            '<button class="chip" data-act="type-unit">Something else&hellip;</button>' +
            '<button class="chip" data-act="hide-count">Don&rsquo;t count them</button>' +
          '</div>'
...
function renderView() {
  var inv = S.inventory;
  if (S.viewFreezer !== 'all') inv = inv.filter(function (l) { return l.freezer === S.viewFreezer; });
  var q = norm(S.viewSearch);
  if (q) inv = inv.filter(function (l) { return norm(l.item).indexOf(q) > -1 || norm(l.category).indexOf(q) > -1; });

  var kinds = {};
  inv.forEach(function (l) { kinds[norm(l.item)] = 1; });

  // The freezer filter doubles as the per-freezer overview: each chip carries
  // its own total, so choosing where to look and seeing what is where are the
  // same glance.
  var allTotal = sum(S.inventory, function (l) { return l.weightG; });
  var chips = '<button class="chip' + (S.viewFreezer === 'all' ? ' is-on' : '') + '" data-act="view-freezer" data-name="all">' +
    ...
    S.freezers.map(function (f) {
      ...
      return '<button class="chip' + (S.viewFreezer === f.name ? ' is-on' : '') + '" data-act="view-freezer" data-name="' + esc(f.name) + '">' +
  ...
  var head = '<div class="stack">' +
    ...
    '</div>' +
    '<div class="seg">' +
      '<button class="seg-btn' + (S.viewMode === 'item' ? ' is-on' : '') + '" data-act="view-mode" data-mode="item">By item</button>' +
      '<button class="seg-btn' + (S.viewMode === 'age' ? ' is-on' : '') + '" data-act="view-mode" data-mode="age">By age</button>' +
    '</div>' +
    '<div class="chips">' + chips + '</div>' +
    '<input class="input" id="view-search" type="search" placeholder="Search for something&hellip;" value="' + esc(S.viewSearch) + '" data-act="view-search">';

  var body = inv.length
    ? (S.viewMode === 'age' ? renderByAge(inv) : renderByItem(inv))
...
/**
 * Oldest first, grouped by the month it went in — the month and year are what
 * actually matter, and the grouping makes "what has been in there longest"
...
function renderByAge(inv) {
  ...
  var more = sorted.length > shown.length
    ? '<button class="btn btn-block" data-act="view-more" style="margin-top:12px">Show more (' + (sorted.length - shown.length) + ' left)</button>'
    : '';

  return '<div class="card card-flush">' +
    '<div class="card-head"><div class="card-title">Oldest first</div><div class="spacer"></div>' +
    '<div class="card-sub">tap a bag to take it out</div></div>' + html + '</div>' + more;
}
...
function renderByItem(inv) {
  ...
  var sections = Object.keys(byCat).sort().map(function (cat) {
    ...
    var rows = Object.keys(byItem).sort().map(function (item) {
      return itemRow(byItem[item], 'i:' + cat + ':' + item, oneFreezer ? S.viewFreezer : null);
    }).join('');
...
function renderTake() {
  ...
  var freezerChips = '<button class="chip' + (S.takeFreezer === 'all' ? ' is-on-warm' : '') + '" data-act="take-freezer" data-name="all">All freezers</button>' +
    S.freezers.map(function (f) {
      return '<button class="chip' + (S.takeFreezer === f.name ? ' is-on-warm' : '') + '" data-act="take-freezer" data-name="' + esc(f.name) + '">' +
        dot(f.colour) + esc(f.name) + '</button>';
    }).join('');
  ...
  var body = shown.length
    ...
    : '<div class="empty"><span class="empty-mark">&#128230;</span>' +
      (S.inventory.length ? 'Nothing here matches those filters.' : 'The freezers are empty.') + '</div>';

  return '<div class="stack">' +
    '<div class="chips">' + freezerChips + '</div>' +
    '<input class="input" id="take-search" type="search" placeholder="Search for something&hellip;" value="' + esc(S.takeSearch) + '" data-act="take-search">' +
    body +
    renderRecentTakes() +
    '</div>';
}
...
function doTake(ids) {
  ...
  var label = lots.length === 1
    ...
    : lots.length + ' bags';
  var sub = 'out of the ' + lots[0].freezer + ' freezer · ' + fmtDate(S.today);
