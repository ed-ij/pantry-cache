...
function renderView() {
  ...
  var head = '<div class="stack">' +
    '<div class="summary">' +
      totalStat(inv) +
      // Entries before unweighed entries: the second qualifies the first, and a
      // subset reads oddly ahead of the set it is part of.
  ...
  var detail = open
    ? '<div class="lots">' + sorted.map(function (l, i) {
        return '<div class="lot' + (S.editing && S.moving[l.id] ? ' is-picked' : '') + '">' +
          (S.editing
            ? '<button class="tick" data-act="toggle-move" ' +
              'data-id="' + esc(l.id) + '" role="checkbox" ' +
              'aria-checked="' + (S.moving[l.id] ? 'true' : 'false') + '" ' +
              'aria-label="Choose this bag to move">' +
              // The same box Take out draws, in a tap-sized target around it,
              // rather than a box blown up to the size of the target.
              '<span class="pick-box">' + (S.moving[l.id] ? '&#10003;' : '') + '</span>' +
              '</button>'
            : '') +
  ...
  var rows = shown.map(function (l) {
    ...
    if (month !== lastMonth) {
      ...
      headHtml = '<div class="group-head">' +
        // The month stays whichever way the setting is set: it is what the
        // list is grouped by, and a heading that stops naming it leaves the
        // page with no calendar date on it anywhere. The age joins it when
        // that is what was asked for.
        (l.dateIn
          ? esc(fmtMonth(l.dateIn)) +
            (S.dateStyle === 'age'
              ? ' <span class="group-meta">' + esc(ageText(l.dateIn)) + '</span>'
              : '')
    ...
    return headHtml +
      '<div class="pick-row' + (on ? ' is-picked' : '') + '">' +
        '<button class="pick" data-act="toggle-pick" data-id="' + esc(l.id) + '" ' +
          ...
          '<span class="row-main">' +
            // Name, place and warning on one line. With the date gone to the
            // heading the second line held a single badge and a lot of air.
            //
            // The place is dropped when the list is already filtered to one, as
            // In store has always done: standing at the Shed, a row that says
            // Shed says nothing, and on a phone it is what pushes the line to
            // wrap.
            '<span class="row-name">' + esc(l.item) +
              (S.store === 'all' ? ' ' + storeBadge(l.store) : '') +
              (first ? ' <span class="badge badge-warm">use first</span>' : '') + '</span>' +
            (l.note || !l.dateIn
              ? '<span class="row-meta">' +
                (l.dateIn ? '' : 'date not recorded') +
                (l.note ? (l.dateIn ? '' : ' &middot; ') + esc(l.note) : '') + '</span>'
              : '') +
          '</span>' +
